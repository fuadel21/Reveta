...
begin
  ...
  if tg_op = 'UPDATE' and (
    ...
    raise exception using
      errcode = '42501',
      message = 'CONVERSATION_PARTICIPANTS_IMMUTABLE';
  end if;
...
before insert or update of buyer_id, seller_id, product_id
...
comment on function public.enforce_conversation_message_privacy() is
  'Enforces seller message privacy on new conversations and prevents participant/product reassignment on existing conversations.';
