...
create policy "products_select_active_owner_admin"
on public.products
for select
to anon, authenticated
...
create policy "transactions_select_parties_or_admin"
...
create policy "transactions_update_parties_or_admin"
on public.transactions
for update
to authenticated
using (buyer_id = auth.uid() or seller_id = auth.uid() or public.is_admin())
...
-- OFFERS: some older DBs may not have seller_id/product_id/message columns, but current app expects them.
...
create policy "product_boosts_select_owner_or_admin"
...
create policy "product_boosts_insert_owner"
on public.product_boosts
for insert
to authenticated
with check (user_id = auth.uid());
...
create policy "product_boosts_update_owner_or_admin"
on public.product_boosts
for update
to authenticated
using (user_id = auth.uid() or public.is_admin())
...
for update
to authenticated
using (bucket_id = 'products' and ((storage.foldername(name))[1] = auth.uid()::text or public.is_admin()))
with check (bucket_id = 'products' and ((storage.foldername(name))[1] = auth.uid()::text or public.is_admin()));
