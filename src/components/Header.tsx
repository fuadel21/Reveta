...
import { Plus, User, Heart, MessageCircle, Search, Shield, Settings, BookMarked, Receipt, MapPin, BarChart3 } from "lucide-react";
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border bg-card/95 backdrop-blur-md supports-[backdrop-filter]:bg-card/80">
      <div className="container flex h-16 items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
          ...
          <Button variant="ghost" size="icon" className="md:hidden text-muted-foreground hover:text-foreground hover:bg-muted/80 transition-all" onClick={() => navigate('/search')}>
          ...
          {user ? (
            <>
              {isAdmin && (
                <Button variant="ghost" size="icon" className="hidden sm:flex text-purple-500 hover:text-purple-600 hover:bg-purple-500/10 transition-all" asChild>
                  <Link to="/admin"><Shield className="h-5 w-5" /></Link>
                </Button>
              )}

              <Button variant="ghost" size="icon" className="hidden sm:flex text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all" asChild>
                <Link to="/profile?tab=favorites"><Heart className="h-5 w-5" /></Link>
              </Button>

              <Button variant="ghost" size="icon" className="hidden sm:flex text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all" asChild>
                <Link to="/messages" aria-label={unreadMessages > 0 ? `${unreadMessages} mensajes sin leer` : 'Mensajes'}>
                  <MessagesIcon />
                </Link>
              </Button>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon" className="text-muted-foreground hover:text-foreground hover:bg-muted/80 transition-all">
                ...
                <DropdownMenuContent align="end" className="w-56">
                  ...
                  <DropdownMenuItem asChild>
                    <Link to="/seller-dashboard" className="flex items-center gap-2"><BarChart3 className="h-4 w-4" />Panel del vendedor</Link>
                  </DropdownMenuItem>
                  <DropdownMenuItem asChild>
                    <Link to="/transactions" className="flex items-center gap-2"><Receipt className="h-4 w-4" />Transacciones</Link>
                  </DropdownMenuItem>
                  <DropdownMenuItem asChild>
                    <Link to="/messages" className="flex items-center gap-2">
                      <MessagesIcon compact />
                      Mensajes
                      {unreadMessages > 0 && <span className="ml-auto text-xs font-bold text-destructive">{unreadMessages > 9 ? '9+' : unreadMessages}</span>}
                    </Link>
                  </DropdownMenuItem>
                  <DropdownMenuItem asChild>
                    <Link to="/saved-searches" className="flex items-center gap-2"><BookMarked className="h-4 w-4" />Búsquedas guardadas</Link>
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem asChild>
                    <Link to="/settings" className="flex items-center gap-2"><Settings className="h-4 w-4" />Ajustes</Link>
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>

              <Button className="h-10 px-4 sm:px-6 rounded-full bg-primary hover:bg-primary/90 text-primary-foreground font-medium shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5" asChild>
                <Link to="/upload"><Plus className="h-5 w-5 sm:mr-2" /><span className="hidden sm:inline">Vender</span></Link>
              </Button>
            </>
          ) : (
            <>
