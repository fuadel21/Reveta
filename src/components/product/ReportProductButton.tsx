...
const ReportProductButton = ({ productId, sellerId, productTitle, isOwner }: ReportProductButtonProps) => {
  ...
  const [reason, setReason] = useState('possible_fraud');
  ...
  const handleSubmit = async () => {
    ...
    if (user.id === sellerId) {
      toast({ title: 'No permitido', description: 'No puedes denunciar tu propio producto.', variant: 'destructive' });
      return;
    }
  ...
  return (
    <>
      <Button variant="outline" className="w-full justify-center border-destructive/30 text-destructive hover:bg-destructive/10" onClick={() => setOpen(true)}>
