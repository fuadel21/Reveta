...
interface ProductSummary { id: string; title: string; images: string[] | null; }
...
export const Chat: React.FC<ChatProps> = ({ productId, sellerId, onClose }) => {
  ...
  const hydrateConversation = useCallback(async (conversation: Conversation): Promise<Conversation> => {
    const [productResult, buyerResult, sellerResult] = await Promise.all([
      supabase.from('products').select('id, title, images').eq('id', conversation.product_id).maybeSingle(),
  ...
  const handleSendOffer = async () => {
    ...
    const amountText = window.prompt('Introduce tu oferta en euros:');
    ...
    <div className="flex flex-col h-full bg-white rounded-lg shadow-lg overflow-hidden border border-border">
      <div className="flex items-center justify-between p-4 border-b bg-primary text-primary-foreground">
        <div className="flex items-center gap-3">
          ...
          <div><h3 className="font-semibold leading-none">{otherUser?.full_name || 'Chat'}</h3>{selectedConversation?.product && <p className="text-xs text-primary-foreground/80 mt-1 truncate max-w-[200px]">{selectedConversation.product.title}</p>}</div>
        </div>
