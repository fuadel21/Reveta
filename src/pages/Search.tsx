...
const SearchPage = () => {
  ...
  useEffect(() => {
    fetchProducts();
  }, [searchParams, geolocation.latitude, geolocation.longitude]);
  ...
  // Auto-request location when geo filter is enabled
  useEffect(() => {
    if (useGeoFilter && !geolocation.hasLocation && !geolocation.loading) {
      geolocation.requestLocation();
    }
  }, [useGeoFilter]);

  // Auto-switch sort to "distance" when geo activates, and away from it when geo deactivates
  useEffect(() => {
  ...
  const fetchProducts = async () => {
    ...
    // Use geo function if enabled and we have location
    if (geoEnabled && geolocation.latitude && geolocation.longitude) {
      ...
      if (error) {
        ...
        // Filter by subcategory client-side if needed (since RPC doesn't support it yet)
        let filteredData = data || [];
        if (subcategoryFilter) {
          filteredData = filteredData.filter((p: Product) => p.subcategory_id === subcategoryFilter);
        }
      ...
      if (maxPrice && Number(maxPrice) < 10000) {
  ...
  const hasActiveFilters = searchQuery || selectedCategory || selectedSubcategory || location || 
    priceRange[0] > 0 || priceRange[1] < 10000 || condition || useGeoFilter;
  ...
  const currentFilters = {
    ...
    radius_km: useGeoFilter ? distanceRadius : undefined,
  };
  ...
  return (
    <>
      <Helmet>
        <title>
          {searchQuery ? `${searchQuery} - Buscar` : 'Explorar productos'} | Reveta
        </title>
        <meta 
          name="description" 
          content="Encuentra productos de segunda mano cerca de ti. Filtra por categoría, precio y ubicación." 
        />
      </Helmet>
        ...
        <main className="flex-1 container py-6">
              ...
              <SheetContent side="right" className="overflow-y-auto">
                ...
                <div className="mt-6">
                  <SearchFilters
                    ...
                    setUseGeoFilter={setUseGeoFilter}
          ...
          <div className="flex gap-8">
            ...
            <aside className="hidden lg:block w-72 shrink-0">
              <div className="sticky top-24 bg-card rounded-xl p-6 border border-border/50 shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                  ...
                  <p className="text-muted-foreground">
                    ...
                    {useGeoFilter && geolocation.hasLocation && ` a menos de ${distanceRadius} km`}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <SortSelect 
                    ...
                    showDistanceOption={useGeoFilter && geolocation.hasLocation}
                  />
                  <ViewModeToggle 
                    viewMode={viewMode}
              ...
              <ActiveFilters
                ...
                onRemoveGeo={() => { setUseGeoFilter(false); applyFilters(); }}
                onRemoveSearch={() => { setSearchQuery(''); applyFilters(); }}
                onRemoveCategory={() => { setSelectedCategory(''); setSelectedSubcategory(''); applyFilters(); }}
                onRemoveSubcategory={() => { setSelectedSubcategory(''); applyFilters(); }}
                onRemoveLocation={() => { setLocation(''); applyFilters(); }}
                onRemoveCondition={() => { setCondition(''); applyFilters(); }}
                onRemovePrice={() => { setPriceRange([0, 10000]); applyFilters(); }}
              />
              ...
              {loading ? (
                ...
                <div className="text-center py-12 bg-muted/30 rounded-xl">
                  ...
                  <p className="text-muted-foreground mb-4">
                    {useGeoFilter 
                      ? 'No hay productos cerca de ti. Prueba aumentar el radio de búsqueda.'
                ...
                <ProductsMap
                  products={mappableProducts as any}
                ...
                <div className="grid grid-cols-2 gap-6 h-[700px]">
                  ...
                  <div className="overflow-y-auto pr-2 scrollbar-thin">
                    <ProductGrid
                      ...
                      selectedProductId={hoveredProductId}
