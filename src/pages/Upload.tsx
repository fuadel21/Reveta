...
const Upload = () => {
  ...
  const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (validFiles.length > remainingSlots) {
      toast({ title: 'Límite de imágenes', description: `Se añadieron ${acceptedFiles.length} foto${acceptedFiles.length === 1 ? '' : 's'}. El máximo es ${MAX_IMAGES}.` });
    }
  ...
  const previewPrice = parsePrice(formData.price);
  ...
  return <>
    ...
    <div className="min-h-screen flex flex-col bg-background"><Header /><main className="flex-1 container py-8">
      ...
      <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[1fr_340px]"><Card className="border-border/50"><CardHeader><CardTitle className="flex items-center gap-2"><UploadIcon className="h-5 w-5" />Crear anuncio</CardTitle><CardDescription>La primera foto será la principal. Puedes cambiar el orden antes de publicar.</CardDescription></CardHeader><CardContent><form onSubmit={handleSubmit} className="space-y-7">
        <section className="space-y-4"><Label className="text-base font-semibold">1. Fotografías *</Label><div className="grid grid-cols-2 gap-4 sm:grid-cols-5">{imageUrls.map((url, index) => <div key={url} className="relative aspect-square overflow-hidden rounded-xl border bg-muted"><img src={url} alt={`Foto ${index + 1}`} className="h-full w-full object-cover" />{index === 0 && <Badge className="absolute left-1 top-1">Principal</Badge>}<button type="button" onClick={() => removeImage(index)} disabled={uploading} className="absolute right-1 top-1 rounded-full bg-destructive p-1 text-white disabled:opacity-40"><X className="h-4 w-4" /></button><div className="absolute bottom-1 left-1 right-1 flex justify-between"><button type="button" disabled={uploading || index === 0} onClick={() => moveImage(index, -1)} className="rounded-full bg-black/60 p-1 text-white disabled:opacity-30"><ArrowUp className="h-4 w-4 -rotate-90" /></button><button type="button" disabled={uploading || index === imageUrls.length - 1} onClick={() => moveImage(index, 1)} className="rounded-full bg-black/60 p-1 text-white disabled:opacity-30"><ArrowDown className="h-4 w-4 -rotate-90" /></button></div></div>)}</div>{images.length < MAX_IMAGES && <div className="grid gap-3 sm:grid-cols-2"><Button type="button" variant="outline" className="h-14 justify-center gap-2 text-base" onClick={() => cameraInputRef.current?.click()} disabled={uploading}><Camera className="h-5 w-5" />Hacer foto</Button><Button type="button" variant="outline" className="h-14 justify-center gap-2 text-base" onClick={() => galleryInputRef.current?.click()} disabled={uploading}><Images className="h-5 w-5" />Elegir de galería</Button><input ref={cameraInputRef} type="file" accept="image/*" capture="environment" onChange={handleImageChange} className="hidden" aria-label="Hacer una foto con la cámara" disabled={uploading} /><input ref={galleryInputRef} type="file" accept="image/jpeg,image/png,image/webp" multiple onChange={handleImageChange} className="hidden" aria-label="Elegir fotos de la galería" disabled={uploading} /></div>}<p className="text-xs text-muted-foreground">Puedes añadir hasta {MAX_IMAGES} fotos. La cámara usa preferentemente la lente trasera del móvil.</p></section>
        ...
        <section className="space-y-4"><Label className="text-base font-semibold">2. Información del producto</Label><div className="space-y-2"><Label htmlFor="title">Título *</Label><Input id="title" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} maxLength={100} placeholder="Ej. iPhone 13 128GB azul" disabled={uploading} /><p className="text-right text-xs text-muted-foreground">{formData.title.length}/100</p></div><div className="space-y-2"><Label htmlFor="description">Descripción *</Label><Textarea id="description" value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} rows={6} maxLength={MAX_DESCRIPTION_LENGTH} placeholder="Estado, antigüedad, accesorios, defectos y motivo de venta." disabled={uploading} /><p className="text-right text-xs text-muted-foreground">{formData.description.length}/{MAX_DESCRIPTION_LENGTH}</p></div></section>
        <section className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="price">Precio *</Label><div className="relative"><Euro className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input id="price" type="text" inputMode="decimal" value={formData.price} onChange={e => setFormData({ ...formData, price: e.target.value })} className="pl-10" placeholder="12,50" disabled={uploading} /></div></div><div className="space-y-2"><Label>Estado *</Label><Select value={formData.condition} onValueChange={value => setFormData({ ...formData, condition: value })} disabled={uploading}><SelectTrigger><SelectValue placeholder="Estado del producto" /></SelectTrigger><SelectContent>{Object.entries(conditionLabels).map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div></section>
        <section className="space-y-4"><Label className="text-base font-semibold">3. Categoría y ubicación</Label><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label>Categoría *</Label><Select value={formData.category_id} onValueChange={value => setFormData({ ...formData, category_id: value, subcategory_id: '' })} disabled={uploading}><SelectTrigger><SelectValue placeholder="Selecciona categoría" /></SelectTrigger><SelectContent>{categories.map(category => <SelectItem key={category.id} value={category.id}>{category.name}</SelectItem>)}</SelectContent></Select></div>{formData.category_id && subcategories.length > 0 && <div className="space-y-2"><Label>Subcategoría</Label><Select value={formData.subcategory_id} onValueChange={value => setFormData({ ...formData, subcategory_id: value })} disabled={uploading}><SelectTrigger><SelectValue placeholder="Selecciona subcategoría" /></SelectTrigger><SelectContent>{subcategories.map(subcategory => <SelectItem key={subcategory.id} value={subcategory.id}>{subcategory.name}</SelectItem>)}</SelectContent></Select></div>}</div><div className="space-y-3"><Label htmlFor="location">Ubicación *</Label><div className="relative"><MapPin className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input id="location" value={formData.location} onChange={e => setFormData({ ...formData, location: e.target.value, latitude: null, longitude: null })} placeholder="Ciudad" className="pl-10" disabled={uploading} /></div><Button type="button" variant={useCurrentLocation && geolocation.hasLocation ? 'default' : 'outline'} size="sm" onClick={() => { if (!useCurrentLocation) { setUseCurrentLocation(true); geolocation.requestLocation(); } else setUseCurrentLocation(false); }} disabled={uploading || geolocation.loading}>{geolocation.loading ? 'Obteniendo...' : useCurrentLocation && geolocation.hasLocation ? <><Locate className="mr-2 h-4 w-4" />Ubicación detectada</> : <><Navigation className="mr-2 h-4 w-4" />Usar mi ubicación</>}</Button></div></section>
        <div className="flex flex-col gap-3 border-t pt-6 sm:flex-row"><Button type="button" variant="outline" onClick={() => setPreviewOpen(true)} className="flex-1" disabled={uploading}><Eye className="mr-2 h-4 w-4" />Vista previa</Button><Button type="button" variant="outline" onClick={() => { localStorage.setItem(DRAFT_KEY, JSON.stringify({ formData, savedAt: new Date().toISOString() })); setLastSavedAt(new Date()); toast({ title: 'Borrador guardado' }); }} className="flex-1" disabled={uploading}><Save className="mr-2 h-4 w-4" />Guardar borrador</Button><Button type="submit" className="flex-1 gradient-hero" disabled={uploading}>{uploading ? 'Publicando...' : 'Publicar anuncio'}</Button></div>
