...
const Upload = () => {
  ...
  const fetchSubcategories = async (categoryId: string) => { const { data, error } = await supabase.from('subcategories').select('*').eq('category_id', categoryId).order('name'); setSubcategories(error ? [] : data || []); };
  ...
  const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (images.length + files.length > 5) { toast({ title: 'Límite de imágenes', description: 'Puedes subir hasta 5 fotos por producto.', variant: 'destructive' }); return; }
    setImages(prev => [...prev, ...files]);
    setImageUrls(prev => [...prev, ...files.map(file => URL.createObjectURL(file))]);
  };

  const removeImage = (index: number) => { setImages(prev => prev.filter((_, i) => i !== index)); setImageUrls(prev => prev.filter((_, i) => i !== index)); };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      navigate('/profile');
    } catch (error: any) { toast({ title: 'No se pudo publicar', description: error?.message || 'Inténtalo de nuevo.', variant: 'destructive' }); }
