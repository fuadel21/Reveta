...
const Upload = () => {
  ...
  const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (remainingSlots === 0 || selectedFiles.length === 0) return;

    const validFiles = selectedFiles.filter(isValidImageFile);
    const acceptedFiles = validFiles.slice(0, remainingSlots);
    const invalidCount = selectedFiles.length - validFiles.length;
    const overflowCount = Math.max(0, validFiles.length - acceptedFiles.length);
    ...
    if (overflowCount > 0) toast({ title: 'Límite de imágenes', description: `Se añadieron ${acceptedFiles.length} foto${acceptedFiles.length === 1 ? '' : 's'} para completar el máximo de ${MAX_IMAGES}.` });
