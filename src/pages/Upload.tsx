...
const Upload = () => {
  ...
  const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    event.target.value = '';
    if (images.length + selectedFiles.length > MAX_IMAGES) { toast({ title: 'Límite de imágenes', description: `Puedes subir hasta ${MAX_IMAGES} fotos por producto.`, variant: 'destructive' }); return; }
  ...
  const removeImage = (index: number) => { if (imageUrls[index]) URL.revokeObjectURL(imageUrls[index]); setImages(prev => prev.filter((_, i) => i !== index)); setImageUrls(prev => prev.filter((_, i) => i !== index)); };
