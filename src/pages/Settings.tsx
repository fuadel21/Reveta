...
const normalizeSettings = (data: Partial<UserSettings> | null | undefined): UserSettings => ({
  ...
  allow_messages_from: allowedMessageScopes.has(String(data?.allow_messages_from))
    ? data?.allow_messages_from as UserSettings['allow_messages_from']
    : DEFAULT_SETTINGS.allow_messages_from,
});
...
const Settings = () => {
  ...
  const fetchAccountData = async () => {
    ...
    if (settingsResult.error) {
      ...
      toast({ title: 'No se pudieron cargar las preferencias', description: 'Se muestran los valores por defecto.', variant: 'destructive' });
    }
  ...
  const fetchBlockedProfiles = async () => {
    ...
    (data || []).forEach((row: any) => {
      profiles[row.id] = { full_name: row.full_name, avatar_url: row.avatar_url };
    });
  ...
  const handleSaveSettings = async () => {
    ...
    const { error } = await supabase
      .from('user_settings')
      .upsert({ user_id: user.id, ...safeSettings });
  ...
  const handlePushPreferenceChange = async (subscribed: boolean) => {
    ...
    const { error } = await supabase
      .from('user_settings')
      .upsert({ user_id: user.id, push_notifications: subscribed } as any);
    if (error) {
  ...
  const handleSaveProfile = async () => {
    ...
    const { data, error } = await supabase
      ...
      .update(payload)
      .eq('id', user.id)
      .select('id,username,full_name,avatar_url,location,phone,bio,verified')
      .maybeSingle();

    if (error) {
      console.error('Error saving profile:', error);
      toast({ title: 'No se pudo guardar el perfil', description: error.code === '23505' ? 'Ese nombre de usuario ya está en uso.' : undefined, variant: 'destructive' });
    } else if (data) {
  ...
  const handlePasswordReset = async () => {
    ...
    if (error) {
      ...
      toast({ title: 'Enlace enviado', description: 'Revisa tu email para establecer una nueva contraseña.' });
    }
        ...
        <main className="flex-1 container py-8 max-w-5xl">
          ...
          <Tabs value={section} onValueChange={handleSectionChange} className="space-y-6">
            ...
            <TabsContent value="profile" className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle>Identidad de la cuenta</CardTitle>
                  <CardDescription>Estos datos alimentan tu perfil privado y, donde corresponda, tu perfil público.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-2"><Label htmlFor="full_name">Nombre completo</Label><Input id="full_name" value={profileForm.full_name} maxLength={80} onChange={(event) => setProfileForm((current) => ({ ...current, full_name: event.target.value }))} /></div>
                    <div className="space-y-2"><Label htmlFor="username">Nombre de usuario</Label><Input id="username" value={profileForm.username} placeholder="usuario" maxLength={30} autoCapitalize="none" onChange={(event) => setProfileForm((current) => ({ ...current, username: event.target.value }))} /><p className="text-xs text-muted-foreground">3-30 caracteres: letras, números, punto, guion o guion bajo.</p></div>
                    <div className="space-y-2"><Label htmlFor="location">Ubicación</Label><Input id="location" value={profileForm.location} placeholder="Ciudad, País" maxLength={120} onChange={(event) => setProfileForm((current) => ({ ...current, location: event.target.value }))} /></div>
                    <div className="space-y-2"><Label htmlFor="phone">Teléfono privado</Label><Input id="phone" value={profileForm.phone} placeholder="+34 600 000 000" maxLength={18} inputMode="tel" onChange={(event) => setProfileForm((current) => ({ ...current, phone: event.target.value }))} /><p className="text-xs text-muted-foreground">No aparece en tu perfil público.</p></div>
                  </div>

                  <div className="space-y-2"><Label htmlFor="bio">Biografía pública</Label><Textarea id="bio" value={profileForm.bio} rows={4} maxLength={MAX_BIO_LENGTH} placeholder="Cuéntanos sobre ti..." onChange={(event) => setProfileForm((current) => ({ ...current, bio: event.target.value }))} /><div className="flex justify-between gap-3 text-xs text-muted-foreground"><span>No publiques teléfono, email, WhatsApp, Bizum ni enlaces externos.</span><span>{profileForm.bio.length}/{MAX_BIO_LENGTH}</span></div></div>

                  <div className="rounded-xl border bg-muted/30 p-4">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <div><p className="font-medium">Email de acceso</p><p className="text-sm text-muted-foreground">{user.email || 'Sin email disponible'}</p></div>
                      <Badge variant={user.email_confirmed_at ? 'default' : 'secondary'}>{user.email_confirmed_at ? 'Email verificado' : 'Pendiente de verificar'}</Badge>
                    </div>
                  </div>

                  <div className="flex flex-wrap gap-2">
                    <Button onClick={() => void handleSaveProfile()} disabled={savingProfile}><Save className="mr-2 h-4 w-4" />{savingProfile ? 'Guardando...' : 'Guardar perfil'}</Button>
                    <Button variant="outline" asChild><Link to="/profile"><ExternalLink className="mr-2 h-4 w-4" />Ver mi área de perfil</Link></Button>
                  </div>
                </CardContent>
              </Card>
            </TabsContent>
            ...
            <TabsContent value="notifications" className="space-y-6">
              <Card>
                <CardHeader><CardTitle>Notificaciones</CardTitle><CardDescription>El navegador solo pedirá permiso cuando actives push desde este control.</CardDescription></CardHeader>
                <CardContent className="space-y-5">
                  <PushNotificationToggle onSubscriptionChange={handlePushPreferenceChange} />
                  <Separator />
                  <div className="flex items-center justify-between gap-4"><div><Label>Notificaciones por email</Label><p className="text-sm text-muted-foreground">Recibe avisos relacionados con tu actividad.</p></div><Switch checked={settings.email_notifications} onCheckedChange={(checked) => updateSetting('email_notifications', checked)} /></div>
                  <Separator />
                  <div className="flex items-center justify-between gap-4"><div><Label>Mensajes nuevos</Label><p className="text-sm text-muted-foreground">Avisar cuando recibas un mensaje.</p></div><Switch checked={settings.message_notifications} onCheckedChange={(checked) => updateSetting('message_notifications', checked)} /></div>
                  <Separator />
                  <div className="flex items-center justify-between gap-4"><div><Label>Ofertas recibidas</Label><p className="text-sm text-muted-foreground">Avisar cuando haya una nueva oferta o cambio relevante.</p></div><Switch checked={settings.offer_notifications} onCheckedChange={(checked) => updateSetting('offer_notifications', checked)} /></div>
                  <Separator />
                  <div className="flex items-center justify-between gap-4"><div><Label>Búsquedas guardadas</Label><p className="text-sm text-muted-foreground">Avisar cuando aparezcan productos que encajen.</p></div><Switch checked={settings.saved_search_notifications} onCheckedChange={(checked) => updateSetting('saved_search_notifications', checked)} /></div>
                  <div className="flex flex-wrap gap-2 pt-2"><Button onClick={() => void handleSaveSettings()} disabled={savingSettings}><Save className="mr-2 h-4 w-4" />{savingSettings ? 'Guardando...' : 'Guardar preferencias'}</Button><Button variant="outline" asChild><Link to="/notifications"><Bell className="mr-2 h-4 w-4" />Abrir bandeja</Link></Button></div>
                </CardContent>
              </Card>
            </TabsContent>
            ...
            <TabsContent value="privacy" className="space-y-6">
              <Card>
                <CardHeader><CardTitle>Privacidad y contacto</CardTitle><CardDescription>Decide qué presencia muestras y quién puede iniciar conversaciones contigo.</CardDescription></CardHeader>
                <CardContent className="space-y-5">
                  <div className="flex items-center justify-between gap-4"><div><Label>Mostrar estado en línea</Label><p className="text-sm text-muted-foreground">Permite que otros usuarios sepan si estás conectado.</p></div><Switch checked={settings.show_online_status} onCheckedChange={(checked) => updateSetting('show_online_status', checked)} /></div>
                  <Separator />
                  <div className="flex items-center justify-between gap-4"><div><Label>Mostrar última conexión</Label><p className="text-sm text-muted-foreground">Permite mostrar cuándo estuviste activo por última vez.</p></div><Switch checked={settings.show_last_seen} onCheckedChange={(checked) => updateSetting('show_last_seen', checked)} /></div>
                  <Separator />
                  <div className="grid gap-2 sm:grid-cols-[1fr_220px] sm:items-center">
                    <div><Label>Quién puede escribirte</Label><p className="text-sm text-muted-foreground">Limita el inicio de nuevas conversaciones.</p></div>
                    <Select value={settings.allow_messages_from} onValueChange={(value) => updateSetting('allow_messages_from', value as UserSettings['allow_messages_from'])}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="everyone">Todos los usuarios</SelectItem><SelectItem value="verified">Solo verificados</SelectItem><SelectItem value="none">Nadie</SelectItem></SelectContent></Select>
                  </div>
                  <div className="rounded-xl border bg-muted/30 p-3 text-sm text-muted-foreground flex gap-2"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><span>Tu teléfono privado no se muestra en perfiles públicos. Usa el chat de Reveta para negociar de forma segura.</span></div>
              ...
              <Card>
                <CardHeader><CardTitle className="flex items-center gap-2"><UserX className="h-5 w-5" />Usuarios bloqueados</CardTitle><CardDescription>Esta lista usa el mismo bloqueo que impide interacciones en el marketplace.</CardDescription></CardHeader>
                <CardContent>
                  {blockedLoading ? <div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div> : blockedUsers.length === 0 ? <p className="py-6 text-center text-sm text-muted-foreground">No has bloqueado a ningún usuario.</p> : <div className="space-y-3">{blockedUsers.map((blocked) => { const blockedProfile = blockedProfiles[blocked.blocked_user_id]; return <div key={blocked.id} className="flex items-center justify-between gap-3 rounded-xl border p-3"><div className="flex min-w-0 items-center gap-3"><div className="h-10 w-10 shrink-0 overflow-hidden rounded-full bg-primary/10 flex items-center justify-center">{blockedProfile?.avatar_url ? <img src={blockedProfile.avatar_url} alt="" className="h-full w-full object-cover" /> : blockedProfile?.full_name?.[0]?.toUpperCase() || 'U'}</div><div className="min-w-0"><p className="truncate font-medium">{blockedProfile?.full_name || 'Usuario de Reveta'}</p><Button variant="link" size="sm" className="h-auto p-0 text-xs" asChild><Link to={`/usuario/${encodeURIComponent(blocked.blocked_user_id)}`}>Ver perfil</Link></Button></div></div><Button variant="outline" size="sm" disabled={unblockingId === blocked.blocked_user_id} onClick={() => void handleUnblock(blocked.blocked_user_id)}>Desbloquear</Button></div>; })}</div>}
            ...
            <TabsContent value="security" className="space-y-6">
              <Card>
                <CardHeader><CardTitle>Acceso y contraseña</CardTitle><CardDescription>Protege el acceso a tu cuenta y mantén tu email actualizado.</CardDescription></CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <p className="text-xs text-muted-foreground">El enlace abre la ruta segura de restablecimiento de Reveta y caduca según la configuración de autenticación.</p>
                </CardContent>
              </Card>

              <Card className="border-destructive/30 bg-destructive/5">
                <CardHeader><CardTitle className="flex items-center gap-2 text-destructive"><Trash2 className="h-5 w-5" />Eliminar cuenta</CardTitle><CardDescription>Esta acción es permanente y puede eliminar o anonimizar tus datos según la política de Reveta.</CardDescription></CardHeader>
                <CardContent><Button variant="destructive" onClick={() => setDeleteDialogOpen(true)}><Trash2 className="mr-2 h-4 w-4" />Solicitar eliminación de cuenta</Button></CardContent>
              </Card>
            </TabsContent>
          </Tabs>
        </main>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={(open) => { setDeleteDialogOpen(open); if (!open) setDeleteConfirmation(''); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Esta acción no se puede deshacer. Se cerrará tu sesión y se procesará la eliminación de tu cuenta. Para confirmar, escribe <strong>ELIMINAR</strong>.</AlertDialogDescription>
          </AlertDialogHeader>
          <div className="space-y-2"><Label htmlFor="delete-confirmation">Confirmación</Label><Input id="delete-confirmation" value={deleteConfirmation} onChange={(event) => setDeleteConfirmation(event.target.value.trim().toUpperCase())} placeholder="ELIMINAR" autoComplete="off" /></div>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deletingAccount}>Cancelar</AlertDialogCancel>
            <AlertDialogAction disabled={deletingAccount || deleteConfirmation !== DELETE_CONFIRMATION} onClick={handleDeleteAccount} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">{deletingAccount ? 'Eliminando...' : 'Eliminar cuenta'}</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
