...
const Transactions = () => {
            ...
            <div className="flex-1 min-w-0">
              ...
              {(hasSendcloudParcel || shippingAddressText) && (
                <div className="mt-3 rounded-lg border bg-muted/40 p-3 text-xs text-muted-foreground space-y-1">
                  <div className="flex items-center gap-2 font-medium text-foreground"><Truck className="h-4 w-4" /><span>Envío Sendcloud</span></div>
  ...
  return (
    <>
      <Helmet><title>Mis Transacciones | Reveta</title><meta name="description" content="Historial de compras y ventas en Reveta" /></Helmet>
      <div className="min-h-screen flex flex-col bg-background">
        ...
        <main className="flex-1 container py-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2 mt-8">
            <Card className="border-border/50"><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">Total Compras</CardTitle></CardHeader><CardContent><p className="text-2xl font-bold">{purchases.reduce((sum, transaction) => sum + transaction.amount, 0).toLocaleString('es-ES')} €</p><p className="text-sm text-muted-foreground">{purchases.length} transacciones</p></CardContent></Card>
            <Card className="border-border/50"><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground">Total Ventas</CardTitle></CardHeader><CardContent><p className="text-2xl font-bold text-green-600">{sales.reduce((sum, transaction) => sum + transaction.amount, 0).toLocaleString('es-ES')} €</p><p className="text-sm text-muted-foreground">{sales.length} transacciones</p></CardContent></Card>
          </div>
        </main>
        <Footer />
      </div>
