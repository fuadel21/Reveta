...
import { AlertTriangle, BarChart3, Eye, Heart, ImageOff, MessageCircle, Package, Pencil, RefreshCw, RotateCcw, Search, ShoppingBag, Sparkles, Tag, Trash2 } from 'lucide-react';
      ...
      <div className="min-h-screen flex flex-col bg-background">
        ...
        <main className="flex-1 container py-8 space-y-6">
          ...
          {visibleProducts.length === 0 ? <Card><CardContent className="p-10 text-center text-muted-foreground"><Package className="mx-auto mb-3 h-10 w-10 opacity-30" />No hay anuncios que coincidan con estos filtros.</CardContent></Card> : <div className="space-y-4">{visibleProducts.map((product) => {
            ...
            <div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" asChild><Link to={`/product/${product.id}`}><Eye className="mr-1 h-4 w-4" />Ver</Link></Button>{canEdit ? <Button size="sm" variant="outline" asChild><Link to={`/edit-product/${product.id}`}><Pencil className="mr-1 h-4 w-4" />Editar</Link></Button> : <Button size="sm" variant="outline" disabled title={blocked ? 'Resuelve la reserva u operación abierta antes de editar' : 'Los productos vendidos no se pueden editar'}><Pencil className="mr-1 h-4 w-4" />Edición bloqueada</Button>}{canBoost ? <Button size="sm" variant="outline" asChild><Link to={`/boost/${product.id}`}><BarChart3 className="mr-1 h-4 w-4" />Destacar</Link></Button> : <Button size="sm" variant="outline" disabled title={boostActive ? 'Este anuncio ya tiene un destacado activo' : 'Solo se pueden destacar anuncios activos'}><BarChart3 className="mr-1 h-4 w-4" />{boostActive ? 'Ya destacado' : 'Solo anuncios activos'}</Button>}<Button size="sm" variant="outline" asChild><Link to="/messages"><MessageCircle className="mr-1 h-4 w-4" />Mensajes</Link></Button>{product.status === 'inactive' ? <Button size="sm" disabled={updatingId === product.id || blocked} onClick={() => changeProductStatus(product, 'active')}><RotateCcw className="mr-1 h-4 w-4" />Reactivar</Button> : product.status === 'active' ? <Button size="sm" variant="destructive" disabled={updatingId === product.id || blocked} onClick={() => changeProductStatus(product, 'inactive')}><Trash2 className="mr-1 h-4 w-4" />Retirar</Button> : null}</div>
